...
async function main() {
    ...
    await prisma.user.upsert({
        ...
        update: { passwordHash: adminPass, role: "ADMIN", orgId: org.id },
        create: { orgId: org.id, role: "ADMIN", email: "admin@local.com", passwordHash: adminPass },
    });
    // Staff users
    const staffPass = await bcrypt.hash("staff123", 10);
    const staffUser1 = await prisma.user.upsert({
        where: { email: "barbero1@local.com" },
        update: { passwordHash: staffPass, role: "STAFF", orgId: org.id },
        create: { orgId: org.id, role: "STAFF", email: "barbero1@local.com", passwordHash: staffPass },
    });
    const staffUser2 = await prisma.user.upsert({
        where: { email: "barbero2@local.com" },
        update: { passwordHash: staffPass, role: "STAFF", orgId: org.id },
        create: { orgId: org.id, role: "STAFF", email: "barbero2@local.com", passwordHash: staffPass },
    });
    ...
    async function ensureAvail(staffId) {
    ...
    await prisma.user.upsert({
        ...
        update: { passwordHash: custPass, role: "CUSTOMER", orgId: org.id, phone: "+59170000000" },
        create: {
            orgId: org.id,
            role: "CUSTOMER",
